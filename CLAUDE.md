...
| 9 | OpenClaw v2026.4.15 filtra el token interno `NO_REPLY` al usuario | En Hetzner se observó que el agente LLM emitía el sentinel `NO_REPLY` literal en respuestas a Telegram (+ alucinaciones con audio). Es una regresión de esa versión. **Fix: downgrade a v2026.3.8** (`npm install -g openclaw@2026.3.8`), misma versión que Talinda. Además, borrar `workspace/BOOTSTRAP.md` si existe — es un archivo de onboard que debería autoborrarse pero a veces queda y contamina el prompt del agente. |
| 10 | `MOBILE_BASE_URL` comentado/vacío → links Telegram inválidos | La variable se usa en 4 lugares (`messageService.js:646,2171`, `taskService.js:1167`, `supervisorAlerts.js:387`) para generar links `/m/assign`, `/m/task`, `/m/escalation` enviados por Telegram. Si queda vacía cae al fallback `http://localhost:3000` — los empleados reciben el mensaje pero no pueden abrir el link desde su cel. El backend ahora loggea **warning al startup** si está mal configurada. Verificar con: `grep -E "^MOBILE_BASE_URL=https?://.+" .env`. |
| 11 | **Timezone mismatch en 3 capas**: `APP_TIMEZONE` (.env), `TZ` (proceso/OS), Postgres timezone — si no coinciden, el bot crea tareas con `work_date` distinto al que busca. | El helper `getTodayDate()` usa `APP_TIMEZONE` (default `America/Guatemala`). Si el `.env` tiene `APP_TIMEZONE=Europe/Madrid` heredado del template de Talinda pero Postgres corre en `America/Guatemala`, un mensaje a las 23:30 CST (05:30 UTC+2 Madrid) crea una tarea en `2026-04-24` (Madrid) pero `CURRENT_DATE` en Postgres es `2026-04-23`. Resultado: "mis tareas" devuelve vacío. Ocurrió en Hetzner abr-2026. **Fix**: alinear las 3 capas con la zona operativa del cliente (ver gotcha #12). Desde abr-2026 el bot loggea warning al startup si detecta mismatch. |
| 12 | Al deployar en server nuevo: revisar **3 TZ separadas** por server | Cada server debe tener estas 3 alineadas con la zona operativa:<br/>• `.env` → `APP_TIMEZONE=<zone>` y `TZ=<zone>`<br/>• Postgres (en el container shared_postgres): `ALTER SYSTEM SET timezone='<zone>'; SELECT pg_reload_conf();`<br/>• systemd del bot: `sudo systemctl edit bot-ia-rrhh-backend.service` → agregar `[Service]\nEnvironment=TZ=<zone>`<br/>Ejemplos actuales: **Talinda** = `Europe/Madrid`. **Hetzner** = `America/Guatemala`. Verificar con: `docker exec shared_postgres psql -c "SHOW TimeZone;"`, `grep APP_TIMEZONE .env`, `date`. |
| 13 | El **SOUL.md vive en 3 ubicaciones**, todas tienen que sincronizarse al deploy | OpenClaw NO lee el SOUL.md del repo. Lo lee de `~/.openclaw/SOUL.md` (system prompt principal) y `~/.openclaw/workspace/SOUL.md` (context inyectado al workspace). Si solo se actualiza una de las 3, el agente sigue con instrucciones viejas y puede narrar / desviarse incluso si el repo tiene la versión correcta. Ocurrió en Talinda abr-2026: `git pull` actualizó el repo pero `~/.openclaw/workspace/SOUL.md` quedó con el SOUL.md original (md5 distinto), y el agente narraba en inglés.<br/>**Fix automatizado**: `bash backend/openclaw/deploy-soul.sh` (compara md5 de las 3 ubicaciones, copia si difieren con backup, reinicia openclaw-gateway).<br/>**Verificación manual**: `md5sum backend/openclaw/SOUL.md ~/.openclaw/SOUL.md ~/.openclaw/workspace/SOUL.md` — los 3 deben coincidir. |
